...
class Game(object):
    ...
    def NewOrContinue(self, a, window, MC, battle, walls, fighters, enemies, expReduction, keyItems):#detects if the player chose new game or continue
        if self.a == 1:
            self.DrawNew(window, MC, battle, walls, fighters, enemies, expReduction, keyItems)
        elif self.a == 2:
            self.DrawLoad()


    def DrawNew(self, window, MC, battle, walls, fighters, enemies, expReduction, keyItems):
        ...
        pygame.display.update()
        self.Control(window, MC, battle, walls, fighters, enemies, expReduction, keyItems)

    def DrawLoad(self):
        pass #Will get variable values from database from when the player last saved

    def Control(self, window, MC, battle, walls, fighters, enemies, expReduction, keyItems):
            ...
            for event in pygame.event.get():
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_UP:
                        ...
                        Hodir.encounter(battle, window, fighters, enemies, MC, walls, Hodir.beaten)
                    ...
                    if event.key == pygame.K_SPACE:
                        ...
                        if steven.done == True:
                            ...
                            print('Steven has decided to join your cause')
                                ...
                                if event.key == pygame.K_s:
                                    ...
                                    if enemies[attacked - 1].health <= 0:
                                        ...
                                        if n == 0:
                                            self.itemDrop(drop, itemList, battleItems)
                                            self.battle = False
                                            self.Control(window, MC, battle, walls, fighters, enemies, keyItems)
                                    attacked = 0
...
class boss(object):
    def __init__(self, x, y, width, height, colour, health, maxHealth, damage, defence, speed, dialogue, exp, beaten):
        self.x = x
        self.y = y
        self.width = width
        self.height = height
    ...
    def bossFightControl(self, fighters, enemies, beaten):
        attacked = 0
        n = 1
                            ...
                            for event in pygame.event.get():
                                if event.type == pygame.KEYDOWN:
                                    if event.key == pygame.K_a:
                                        ...
                                        #put animation here
                                        damageGiven =  round(40 + (fighter.damage // enemy.defence))
                                        enemies[attacked - 1].health -= damageGiven
                                        moved = True
...
class NPC(object):
    def __init__(self, x, y, width, height, colour, dialogueA, dialogueB, request, reward, done, coins):
        self.x = x
        self.y = y
        self.width = width
        self.height = height
...
scroll = overworldItem(300, 500, 10, 10, 'scroll', False)

keyItems = []
battleItems = []


player = fighter(300, 200, 30, 80, 1, 3000, 3000, 15, 1, 12, 0, black, 'A: attack, S: ice spell, M:  item menu')
steve = fighter(300, 300, 30, 80, 3, 150, 150, 19, 3, 12, 0, DBlue, 'A: attack, S: electricity spell, M: item menu')
akechi = fighter(300, 400, 30, 80, 7, 200, 200, 25, 6, 20, 0, white, 'A: attack, S: blessed spell, M: item menu')
maria = fighter(300, 500, 30, 80, 10, 250, 250, 29, 9, 30, 0, darkRed, 'A: attack, S: fire spell, M: item menu')

boi1 = enemy(1000, 200, 30, 80, 1, 100, 100, 11, 1.2, randint(9,12), 30, green)
boi2 = enemy(1000, 400, 30, 80, 1, 100, 100, 11, 1.2, randint(9,12), 30, green)
boi3 = enemy(1000, 600, 30, 80, 1, 100, 100, 11, 1.2, randint(9,12), 30, green)

potion = battleItem('Heal 100', 'Potion')
atkUp = battleItem('Damage Increase', 'atkUp')
defUp = battleItem('Defence increase', 'defUp')

steven = NPC(X//2 + 50, 300, 30, 80, DBlue, 'If you want to know some of what is happening within this twisted world, you will want to fetch me the scroll from the town square. You will be rewarded', 'Glaive Master Hodir is a man who has been cursed for all eternity to test those who come through here. Those who can best him, in his eyes, may be able to succeed where he could not.', 'scroll', 'Right eye', False, 100)
...
Hodir = boss((X//2 - 115), -4300, 70, 120, DBlue, 600, 600, 15, 2.6, 16, 'Hand it over. That thing. Your Dark Sock', 150, False)
...
game = Game(a, False)
game.NewOrContinue(a, window, MC, False, walls, fighters, enemies, expReduction, keyItems)
